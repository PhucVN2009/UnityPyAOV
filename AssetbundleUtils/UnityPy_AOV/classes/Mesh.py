...
class Mesh(NamedObject):
    ...
    def save(self, writer=None):
        """Serialize Mesh object lại thành binary và lưu vào reader.data."""
        from ..export.MeshImporter import _serialize_mesh_with_new_geo
        version = self.version
        endian = self.reader.endian
        vc = getattr(self, "m_VertexCount", len(self.m_Vertices) // 3)
        raw = _serialize_mesh_with_new_geo(
            self, version, endian,
            self.m_Vertices, self.m_Normals, self.m_UV0, self.m_Tangents,
            self.m_IndexBuffer, [],
            vc,
        )
        self.reader.data = raw
