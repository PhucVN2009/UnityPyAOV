        ...
        for index in range(0, end, chunkStride // 4):
            for i in range(itemCountInChunk):
                ...
                while bits < self.m_BitSize:
                    if indexPos >= len(self.m_Data):  # guard corrupt data
                        break
                    x |= uint(
...
class PackedIntVector:
    ...
    def UnpackInts(self):
        ...
        if self.m_BitSize == 0 or self.m_NumItems == 0:
            return [0] * self.m_NumItems
        data = [0] * self.m_NumItems
        indexPos = 0
        bitPos = 0
        data_len = len(self.m_Data)
        for i in range(self.m_NumItems):
            bits = 0
            data[i] = 0
            while bits < self.m_BitSize:
                if indexPos >= data_len:  # guard against corrupt data
                    break
...
class PackedQuatVector:
    ...
    def UnpackQuats(self):
        m_Data = self.m_Data
        data_len = len(m_Data)
        # FIX: use data[i] = q instead of data.append(q) (append caused data[i]=None)
        data = [Quaternion() for _ in range(self.m_NumItems)]
        indexPos = 0
        bitPos = 0
        ...
        for i in range(self.m_NumItems):
            ...
            bits = 0
            while bits < 3:
                if indexPos >= data_len: break  # guard corrupt data
                ...
                if (flags & 3) != j:
                    ...
                    while bits < bitSize:
                        ...
                        if bitPos == 8:
                            ...
                            bitPos = 0
                    x &= (1 << bitSize) - 1
                    denom = 0.5 * ((1 << bitSize) - 1)
                    q[j] = x / denom - 1 if denom else 0.0
                    sum_sq += q[j] * q[j]

            lastComponent = flags & 3
            inner = max(0.0, 1 - sum_sq)  # clamp to avoid sqrt of negative
            q[lastComponent] = math.sqrt(inner)
            if (flags & 4) != 0:
                q[lastComponent] = -q[lastComponent]
            data[i] = q  # FIX: assign to index, not append
...
class StreamedCurveKey:
    def __init__(self, reader):
        ...
        # Guard: coeff is a tuple from Struct.unpack; may be short if buffer corrupt
        self.outSlope = self.coeff[2] if len(self.coeff) > 2 else 0.0
        self.value    = self.coeff[3] if len(self.coeff) > 3 else 0.0
...
class AnimationClip(NamedObject):
    def __init__(self, reader):
        ...
        if version >= (4,):  # 4.0 and up
            ...
            # CRITICAL: save position trước khi parse để dùng m_MuscleClipSize
            # seek past nếu parse fail — giữ reader position đúng cho
            # m_ClipBindingConstant ở phía sau.
            _muscle_start = reader.Position
            try:
                self.m_MuscleClip = ClipMuscleConstant(reader)
            except Exception:
                self.m_MuscleClip = None
            # Always seek đến vị trí đã biết chính xác dùng size prefix
            _muscle_end = _muscle_start + self.m_MuscleClipSize
            if reader.Position != _muscle_end:
                reader.Position = _muscle_end
        if version >= (4, 3):  # 4.3 and up
            try:
                self.m_ClipBindingConstant = AnimationClipBindingConstant(reader)
            except Exception:
                self.m_ClipBindingConstant = AnimationClipBindingConstant.__new__(AnimationClipBindingConstant)
                self.m_ClipBindingConstant.genericBindings = []
                self.m_ClipBindingConstant.pptrCurveMapping = []
        # Unity 2018.3+: extra fields (not always present)
        if version >= (2018, 3):
            try:
                ...
                # skip events
                reader.Position += numEvents * 36  # approximate event size
            except Exception:
                pass
