...
def import_mesh(lang, tree):
    """Imports an OBJ file into a selected Mesh asset."""
    global selected_items, modified_assets

    if len(selected_items) != 1:
        show_dialog(lang["Dialog_Title"], lang["Dialog_Message_Select_One_Raw"])
        return

    fp = open_file(lang, "Pick_Input_Mesh", "OBJ Mesh files", "*.obj")
    if not fp:
        return
    ...
    for obj in env_list[indexFile].objects:
        if str(obj.path_id) == pathID:
            if obj.type.name != "Mesh":
                show_dialog(lang["Dialog_Title"], "Asset được chọn không phải Mesh.")
                return
            try:
                from .UnityPy_AOV.export.MeshImporter import import_obj_to_mesh
                import_obj_to_mesh(obj, fp)
                modified_assets[str(pathID)] = "*"
                refresh_modified_status(tree)
                show_dialog(lang["Dialog_Title"], lang["Dialog_Message_Import_Success"])
            except Exception as e:
                import traceback
                traceback.print_exc()
                show_dialog("Error", f"Import Mesh failed:\n{e}")
            return
